...
class RoleController extends Controller
{
    ...
    {
        $resource = Role::with('users')->orderBy('id', 'ASC')->get();
    ...
    public function store(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'libelle' => "required|string|unique:roles,rlelibelle",
        ...
        $resource = Role::create([
            'rlelibelle' => $request->libelle,
            'rledescription' => $request->description,
            'rleactive' => $request->active,
        ]);
        if ($resource) {
    ...
    {
        $resource = Role::with('users')->where("id", $id)->first();
        if ($resource) {
    ...
    {
        $validator = Validator::make($request->all(), [
            'libelle' => "required|string|unique:roles,rlelibelle," . $id,
            'description' => "string|nullable",
            'active' => "string|size:3",
        ]);
        ...
        $resource = Role::findOrFail($id)->update([
            'rlelibelle' => $request->libelle,
            'rledescription' => $request->description,
            'rleactive' => $request->active,
        ]);
    ...
    {
        $resource = ($id === "all") ? Role::truncate() : Role::findOrFail($id)->delete();
